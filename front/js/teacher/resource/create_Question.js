...
function newOption() {
    ...
    imgButton1.src = "/images/resource/delete.png";
    imgButton1.style = "width: 24px; height: 24px;"
    ...
    imgButton2.src = "/images/resource/insert.png";
...
function create_question() {
    ...
    if(flag){
        ...
        for (var i = 0; i < len; i++) {
            data.create_question.options.Options.push({
    ...
    {
        data.create_question.is_public = true;
    }
    else
    {
        data.create_question.is_public = false;
    }
...
function post(data) {
    fetch('/resource', {
        ...
        .then(_data => {
            if(isCreatePaper){  //如果是创建试卷页面，将题目导入试卷
                getData(_data.data.question_id);   //获取题目
            }
            getdata2()
            getdata3()
            hidePopup()
...
function showPopup() {
    ...
    if (!CQ_haveGotSubjects) CQ_getSubjects();
}
...
function CQ_getSubjects() {
            if (subject_list.subjects[0]) CQ_haveGotSubjects = true;
...
function CQ_getCourses(subject) {
            var i = 0;
            console.log(subject);
...
function CQ_getSections(course) {
            var i = 0;
...
dropdown1.addEventListener('change', function() {
    ...
    if (subject) {
        ...
        CQ_getCourses(subject); // 调用 CQ_getCourses 函数并传入课程列表
...
dropdown2.addEventListener('change', function() {
    ...
    if (course) {
        dropdown3.innerHTML = '<option value="">/</option>';
        CQ_getSections(course); // 调用 CQ_getSections 函数并传入课程列表
